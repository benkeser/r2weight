...
Here's an example where we use different predictors to predict `Y1` and `Y2`:
```{r cvslfits2, cache=TRUE}
set.seed(1234)
# simulate a data.frame
X <- data.frame(x1 = runif(100, 0, 1),
                x2 = rnorm(100, 0, 1), 
...
Here's an example showing that we can use `r2weight` even when there is just
a single outcome to predict. Thus, the function can be used as obtaining 
inference on cross-validated $R^2$ after fitting `CV.SuperLearner`:
```{r}
r2FitSingle <- r2weight(cvslList = list(cvsl1), X=list(X))
r2FitSingle
```
We can also get the performance of a single algorithm used by the Super Learner:
```{r}
# 
r2FitGLM <- r2weight(
